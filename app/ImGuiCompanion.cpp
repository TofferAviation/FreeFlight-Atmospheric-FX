...
void overview() {
    ...
    ImGui::BeginChild("activeFx",ImVec2(midLeft,218),true);ImGui::SetWindowFontScale(1.25f);ImGui::Text("Active Effects");ImGui::SetWindowFontScale(1);changed|=toggle("Engine contrails",gSettings.enabled);changed|=ImGui::SliderFloat("Intensity##contrail",&gSettings.contrailIntensity,0.0f,2.0f,"%.0f%%");bool vort=gSettings.wingVortexIntensity>.01f;if(toggle("Wingtip vortices",vort)){gSettings.wingVortexIntensity=vort?1.0f:0.0f;changed=true;}changed|=ImGui::SliderFloat("Intensity##vortex",&gSettings.wingVortexIntensity,0.0f,2.0f,"%.0f%%");bool wing=gSettings.wingCondensationIntensity>.01f;if(toggle("Over-wing vapour",wing)){gSettings.wingCondensationIntensity=wing?1.0f:0.0f;changed=true;}changed|=ImGui::SliderFloat("Intensity##wing",&gSettings.wingCondensationIntensity,0.0f,2.0f,"%.0f%%");ImGui::EndChild();
...
void frame() {
    ...
    switch(gPage){case Page::Overview:overview();break;case Page::Effects:simplePage("Effects Control","Shape every visible vapour layer");break;case Page::Weather:simplePage("Weather & Realism","Atmospheric triggering and persistence logic");break;case Page::Performance:simplePage("Quality & Performance","Particle budget and frame-time protection");break;case Page::Aircraft:simplePage("Aircraft Profile","Emitter calibration and integration status");break;case Page::Advanced:simplePage("Advanced & Test","Diagnostics and forced-effect controls");break;}ImGui::EndChild();ImGui::End();
}
