...
bool effectSlider(const char* label,const char* id,float* value,float minimum,float maximum,const char* suffix,float displayScale=1.0f) {
    ImGui::Text("%s",label);char text[32];std::snprintf(text,sizeof(text),"%.0f%s",*value*displayScale,suffix);float tw=ImGui::CalcTextSize(text).x;ImGui::SameLine(ImGui::GetContentRegionMax().x-tw);ImGui::TextColored(C(62,184,255),"%s",text);return assetSliderFloat(id,value,minimum,maximum);
}
void sidebar(float width) {
    ...
    if(TextureAsset* stripes=asset("backgrounds/login_background_dark_curves.png")){ImVec2 ws=ImGui::GetWindowSize();d->AddImage((ImTextureID)(intptr_t)stripes->view,wp,ImVec2(wp.x+ws.x,wp.y+ws.y),ImVec2(.12f,0),ImVec2(.72f,1),U(255,255,255,145));d->AddRectFilled(wp,ImVec2(wp.x+ws.x,wp.y+ws.y),U(1,11,24,38));}
...
void overview() {
    ...
    ImGui::BeginChild("statusbar",ImVec2(avail,46),true);ImGui::TextColored(gSettings.automaticWeather?C(99,225,103):C(150,168,188),gSettings.automaticWeather?"✓ Weather sync active":"Weather sync disabled");ImGui::SameLine(avail*.38f);ImGui::TextColored(gStatus.pluginRunning?C(99,225,103):C(150,168,188),gStatus.pluginRunning?"◎ Telemetry online":"◎ Waiting for telemetry");ImGui::SameLine(avail*.72f);ImGui::TextDisabled("Atmospheric FX v%s",ffatmo::app::kAppVersion);ImGui::EndChild();if(changed)saveSettings();
}
void effectsPage() {
    ImGui::SetWindowFontScale(1.65f);ImGui::Text("Effects Control");ImGui::SetWindowFontScale(1);ImGui::TextColored(C(57,198,249),"Shape every vapour layer and watch the wake respond live");ImGui::Spacing();
    float avail=ImGui::GetContentRegionAvail().x,gap=14;bool changed=false;float card=(avail-gap*2)/3;
    ImGui::BeginChild("fxMaster",ImVec2(card,112),true);ImGui::TextDisabled("EFFECT SYSTEM");changed|=toggle("Atmospheric effects",gSettings.enabled);ImGui::TextColored(gSettings.enabled?C(98,224,105):C(150,168,188),gSettings.enabled?"LIVE OUTPUT ENABLED":"ALL EMITTERS DISABLED");ImGui::EndChild();ImGui::SameLine(0,gap);
    ImGui::BeginChild("fxPreview",ImVec2(card,112),true);ImGui::TextDisabled("TESTING");changed|=toggle("Preview mode",gSettings.previewMode);ImGui::TextColored(gSettings.previewMode?C(248,184,65):C(150,168,188),gSettings.previewMode?"FORCED CONDITIONS ACTIVE":"REAL WEATHER CONDITIONS");ImGui::EndChild();ImGui::SameLine(0,gap);
    ImGui::BeginChild("fxConditions",ImVec2(card,112),true);ImGui::TextDisabled("LIVE FORMATION");ImGui::SetWindowFontScale(1.45f);ImGui::TextColored(C(53,177,255),"%.0f%%",gStatus.state.formationProbability*100);ImGui::SetWindowFontScale(1);ImGui::SameLine();ImGui::TextDisabled("probability");ImGui::Text("%.0f sec  /  %.1f km",gStatus.state.contrailPersistenceSeconds,gStatus.state.estimatedTrailLengthKm);ImGui::EndChild();
    float left=(avail-gap)*.43f,right=avail-gap-left;
    ImGui::BeginChild("fxControls",ImVec2(left,0),false);
    ImGui::BeginChild("emitters",ImVec2(0,286),true);ImGui::SetWindowFontScale(1.25f);ImGui::Text("Emitter intensity");ImGui::SetWindowFontScale(1);ImGui::TextDisabled("Individual output multipliers");ImGui::Spacing();changed|=effectSlider("Engine contrails","fxContrails",&gSettings.contrailIntensity,0,2,"%",100);ImGui::Spacing();changed|=effectSlider("Wingtip vortices","fxVortices",&gSettings.wingVortexIntensity,0,2,"%",100);ImGui::Spacing();changed|=effectSlider("Over-wing condensation","fxWing",&gSettings.wingCondensationIntensity,0,2,"%",100);ImGui::EndChild();
    ImGui::BeginChild("persistence",ImVec2(0,238),true);ImGui::SetWindowFontScale(1.25f);ImGui::Text("Persistence & triggering");ImGui::SetWindowFontScale(1);ImGui::TextDisabled("Formation altitude and trail lifetime");ImGui::Spacing();changed|=effectSlider("Persistence scale","fxPersistence",&gSettings.persistenceScale,.25f,2,"%",100);ImGui::Spacing();changed|=effectSlider("Minimum contrail altitude","fxAltitude",&gSettings.minimumContrailAltitudeFt,0,50000," ft");ImGui::Spacing();changed|=toggle("Automatic weather logic",gSettings.automaticWeather);ImGui::EndChild();ImGui::EndChild();
    ImGui::SameLine(0,gap);ImGui::BeginChild("fxLive",ImVec2(right,0),true);ImGui::SetWindowFontScale(1.25f);ImGui::Text("Live wake visualization");ImGui::SetWindowFontScale(1);ImGui::TextDisabled("X-Crafts Lineage 1000  /  profile-aware emitters");ImGui::Spacing();drawAircraftWake(ImVec2(ImGui::GetContentRegionAvail().x,330));
    float stage=(ImGui::GetContentRegionAvail().x-gap*3)/4;auto phase=[&](const char* title,float value,ImU32 color){ImGui::BeginChild(title,ImVec2(stage,100),false);ImGui::TextDisabled("%s",title);char pct[16];std::snprintf(pct,sizeof(pct),"%.0f%%",std::clamp(value,0.f,1.f)*100);ImGui::SetWindowFontScale(1.35f);ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(color),"%s",pct);ImGui::SetWindowFontScale(1);ImGui::ProgressBar(std::clamp(value,0.f,1.f),ImVec2(-1,5),"");ImGui::EndChild();};phase("PRIMARY",gStatus.state.primaryWakeRatio,U(39,156,255));ImGui::SameLine(0,gap);phase("CURTAIN",gStatus.state.secondaryCurtainRatio,U(39,207,244));ImGui::SameLine(0,gap);phase("CIRRUS",gStatus.state.contrailCirrusRatio,U(244,181,57));ImGui::SameLine(0,gap);phase("BUDGET",gStatus.state.particleBudgetRatio,U(99,224,105));
...
void loginPage(const ffatmo::app::GithubAuthState& auth) {
    ...
    image("login/branding/freeflight_atmospheric_fx_lockup.png",190,205,650,349);
    image("login/branding/tagline_atmosphere_refined.png",330,588,310,30);
    ...
    const float featureX[]={190,385,575};
    for(int i=0;i<3;++i){image(featureIcons[i],featureX[i],758,42,42);textAt(featureX[i]+48,770,featureNames[i],C(104,188,243),.82f);if(i<2)image("login/controls/feature_divider_vertical.png",featureX[i]+177,750,1,72);}

    const float cardX=925,cardY=141,cardW=595,cardH=659,contentX=1002,contentW=443;
    image("login/panels/login_card_empty.png",cardX,cardY,cardW,cardH);
    textAt(contentX,192,"Welcome back",C(226,237,248),1.75f);
    textAt(contentX,234,"Sign in to continue to Atmospheric FX",C(125,190,234),.88f);

    if(auth.phase==ffatmo::app::GithubAuthPhase::Checking){
        image("login/icons/color/secure_connection.png",contentX,318,54,54);textAt(contentX+72,322,"Connecting securely to GitHub",C(226,237,248),1.08f);textAt(contentX+72,354,"Checking for an encrypted session on this PC...",C(137,162,188),.78f);
    }else if(auth.phase==ffatmo::app::GithubAuthPhase::AwaitingUser){
        textAt(contentX,292,"AUTHORIZE THIS DEVICE",C(226,237,248),.80f);textAt(contentX,322,"Enter this one-time code on the GitHub page:",C(180,204,225),.82f);
        d->AddRectFilled(ImVec2(canvas.x+contentX*scale,canvas.y+366*scale),ImVec2(canvas.x+(contentX+contentW)*scale,canvas.y+466*scale),U(5,25,47,240),12*scale);d->AddRect(ImVec2(canvas.x+contentX*scale,canvas.y+366*scale),ImVec2(canvas.x+(contentX+contentW)*scale,canvas.y+466*scale),U(38,147,225),12*scale,0,1.2f*scale);
        centeredText(contentX,391,contentW,auth.userCode.c_str(),C(66,190,255),2.0f);
        ImGui::SetCursorScreenPos(ImVec2(canvas.x+contentX*scale,canvas.y+493*scale));if(ImGui::Button("COPY CODE",ImVec2(214*scale,52*scale)))ImGui::SetClipboardText(auth.userCode.c_str());ImGui::SetCursorScreenPos(ImVec2(canvas.x+(contentX+229)*scale,canvas.y+493*scale));if(ImGui::Button("OPEN GITHUB",ImVec2(214*scale,52*scale)))gGithubAuth.openVerificationPage();
        centeredText(contentX,570,contentW,"Waiting for approval in your browser...",C(137,162,188),.76f);
    }else{
        textAt(contentX,290,"SIGN IN WITH GITHUB",C(226,237,248),.80f);
        textAt(contentX,317,"Use your own GitHub account. Authentication happens on GitHub,",C(195,214,230),.76f);textAt(contentX,338,"so FreeFlight never sees or stores your password.",C(195,214,230),.76f);
        d->AddRectFilled(ImVec2(canvas.x+contentX*scale,canvas.y+383*scale),ImVec2(canvas.x+(contentX+contentW)*scale,canvas.y+493*scale),U(5,25,47,238),13*scale);d->AddRect(ImVec2(canvas.x+contentX*scale,canvas.y+383*scale),ImVec2(canvas.x+(contentX+contentW)*scale,canvas.y+493*scale),U(39,147,224),13*scale,0,1.2f*scale);
        image("login/icons/color/secure_connection.png",contentX+28,407,44,44);textAt(contentX+92,394,"Secure GitHub authorization",C(226,237,248),.78f);textAt(contentX+92,423,"Browser-based device login",C(137,162,188),.76f);textAt(contentX+92,452,"Encrypted session storage on this PC",C(137,162,188),.76f);
        if(auth.phase==ffatmo::app::GithubAuthPhase::Error)textAt(contentX,500,auth.error.c_str(),C(255,116,116),.72f);
        ImGui::SetCursorScreenPos(ImVec2(canvas.x+contentX*scale,canvas.y+516*scale));ImGui::InvisibleButton("##remember",ImVec2(28*scale,28*scale));if(ImGui::IsItemClicked())gLoginRemember=!gLoginRemember;image(gLoginRemember?"login/controls/checkbox_checked.png":"login/controls/checkbox_unchecked.png",contentX,516,28,28);textAt(contentX+36,521,"Remember me on this computer",C(202,220,235),.76f);
        image("login/buttons/sign_in_button_empty.png",contentX,560,contentW,60);ImGui::SetCursorScreenPos(ImVec2(canvas.x+contentX*scale,canvas.y+560*scale));bool loginClicked=ImGui::InvisibleButton("##githubLogin",ImVec2(contentW*scale,60*scale));if(ImGui::IsItemHovered())d->AddRect(ImVec2(canvas.x+contentX*scale,canvas.y+560*scale),ImVec2(canvas.x+(contentX+contentW)*scale,canvas.y+620*scale),U(255,215,91),12*scale,0,2*scale);centeredText(contentX,580,contentW,"CONTINUE WITH GITHUB",C(239,247,255),.82f);if(loginClicked)gGithubAuth.begin(gLoginRemember);
        centeredText(contentX,654,contentW,"FreeFlight LLC  -  Secure connection",C(121,147,174),.72f);
    }
    textAt(1478,900,(std::string("Atmospheric FX v")+ffatmo::app::kAppVersion).c_str(),C(121,147,174),.72f);
}
void updateModal() {
    ...
    if(ImGui::BeginPopupModal("##update_available",nullptr,ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoResize|ImGuiWindowFlags_NoScrollbar|ImGuiWindowFlags_NoSavedSettings)){
        ...
        ImGui::PushStyleColor(ImGuiCol_Button,C(18,91,219));ImGui::PushStyleColor(ImGuiCol_ButtonHovered,C(30,119,248));ImGui::PushStyleColor(ImGuiCol_Border,C(244,190,54));ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize,1.5f);
        if(ImGui::Button(update.previewOnly?"CLOSE PREVIEW":"UPDATE NOW",ImVec2(buttonWidth,50))){if(update.previewOnly){gUpdateService.dismissCurrent();ImGui::CloseCurrentPopup();}else{std::string error;if(gUpdateService.launchUpdater(executablePath().parent_path().wstring(),&error))PostMessageW(gWindow,WM_CLOSE,0,0);else gFooter=error;}}ImGui::PopStyleVar();ImGui::PopStyleColor(3);
        const char* footer=update.previewOnly?"TEST PREVIEW - no files will be downloaded or installed.":"X-Plane should be closed while plugin files are replaced.";float fw=ImGui::CalcTextSize(footer).x;ImGui::SetCursorPosX((modalSize.x-fw)*.5f);ImGui::TextDisabled("%s",footer);ImGui::EndPopup();
    }
...
void frame() {
    ...
    if(auth.phase!=ffatmo::app::GithubAuthPhase::Authorized){if(TextureAsset* bg=asset("login/backgrounds/login_background_clean.png"))ImGui::GetWindowDrawList()->AddImage((ImTextureID)(intptr_t)bg->view,p,ImVec2(p.x+s.x,p.y+s.y));loginPage(auth);ImGui::End();return;}
    if(TextureAsset* bg=asset("backgrounds/login_background_dark_curves.png"))ImGui::GetWindowDrawList()->AddImage((ImTextureID)(intptr_t)bg->view,p,ImVec2(p.x+s.x,p.y+s.y),ImVec2(0,0),ImVec2(1,1),U(255,255,255,150));
    else if(TextureAsset* bg=asset("backgrounds/background_atmospheric_clean.png"))ImGui::GetWindowDrawList()->AddImage((ImTextureID)(intptr_t)bg->view,p,ImVec2(p.x+s.x,p.y+s.y));
