...
void appendSnapshot(ByteWriter& writer, const engine::SimulatorSnapshot& snapshot) {
    ...
    writer.u32(atmosphere.profile.levelCount);
    appendFloatArray(writer, atmosphere.profile.altitudeMslM);
    appendFloatArray(writer, atmosphere.profile.temperatureK);
    appendFloatArray(writer, atmosphere.profile.dewPointK);
    appendFloatArray(writer, atmosphere.profile.windSpeedMps);
    appendFloatArray(writer, atmosphere.profile.windDirectionDegTrue);
    appendFloatArray(writer, atmosphere.profile.shearSpeedMps);
    appendFloatArray(writer, atmosphere.profile.shearDirectionDegTrue);
    appendFloatArray(writer, atmosphere.profile.turbulenceScale);
...
bool readSnapshot(ByteReader& reader, engine::SimulatorSnapshot& snapshot) {
    ...
    if (!reader.f32(atmosphere.temperatureK) ||
        ...
        !reader.u32(atmosphere.profile.levelCount) ||
        !readFloatArray(reader, atmosphere.profile.altitudeMslM) ||
        !readFloatArray(reader, atmosphere.profile.temperatureK) ||
        !readFloatArray(reader, atmosphere.profile.dewPointK) ||
        !readFloatArray(reader, atmosphere.profile.windSpeedMps) ||
        !readFloatArray(reader, atmosphere.profile.windDirectionDegTrue) ||
        !readFloatArray(reader, atmosphere.profile.shearSpeedMps) ||
        !readFloatArray(reader, atmosphere.profile.shearDirectionDegTrue) ||
        !readFloatArray(reader, atmosphere.profile.turbulenceScale) ||
        !reader.u32(snapshot.engineCount)) {
        return false;
    }

    if (atmosphere.profile.levelCount > engine::kMaximumAtmosphereLevels ||
        snapshot.engineCount > engine::kMaximumRecordedEngines) {
        return false;
    }
    ...
    for (auto& engine : snapshot.engines) {
        if (!reader.u8(engine.running) ||
            ...
            !reader.f32(engine.exhaustVelocityMps)) {
            return false;
        }
    }
...
std::string metadataText(const ReplayMetadata& metadata) {
    ...
    stream << "git_revision=" << metadata.gitRevision << '\n';
    stream << "xplane_version=" << metadata.xplaneVersion << '\n';
    stream << "platform=" << metadata.platform << '\n';
    stream << "aircraft_name=" << metadata.aircraftName << '\n';
    stream << "aircraft_icao=" << metadata.aircraftIcao << '\n';
    stream << "aircraft_relative_path=" << metadata.aircraftRelativePath << '\n';
    stream << "coordinate_convention=" << metadata.coordinateConvention << '\n';
    stream << "scenario_seed=" << metadata.scenarioSeed << '\n';
    return stream.str();
}
...
void writeChunk(std::ofstream& stream,
    ...
    header.u32(0u);  // compression: none
...
bool readFileHeader(std::ifstream& stream, std::string& error) {
    ...
    if (magic != kMagic) {
        error = "Replay magic is not FFATRPL1";
        return false;
    }
    if (containerVersion != kReplayContainerVersion) {
        error = "Unsupported replay container version";
        return false;
    }
    if (snapshotVersion != engine::kSimulatorSnapshotSchemaVersion) {
        error = "Unsupported simulator snapshot schema version";
        return false;
    }
    if (endian != kEndianMarker) {
        error = "Replay endianness is unsupported";
        return false;
    }
    return true;
}
...
bool readChunkHeader(std::ifstream& stream, DecodedChunkHeader& header, bool& endOfFile) {
    endOfFile = false;
    ...
    ByteReader reader(bytes);
    return reader.u32(header.type) &&
           reader.u32(header.schemaVersion) &&
           reader.u64(header.payloadBytes) &&
           reader.u64(header.firstSequence) &&
           reader.u64(header.lastSequence) &&
           reader.u32(header.checksum) &&
...
public:
    bool start(const std::filesystem::path& outputPath,
        ...
        std::vector<std::uint8_t> metadataPayload(text.begin(), text.end());
        writeChunk(stream_, kChunkMetadata, 1u, 0u, 0u, metadataPayload);
        if (!stream_) {
    ...
    bool stop(std::string* error) {
        ...
        endPayload.u64(accepted_.load(std::memory_order_relaxed));
        endPayload.u64(dropped_.load(std::memory_order_relaxed));
    ...
    void workerLoop() {
        ...
        while (running_.load(std::memory_order_acquire) ||
            ...
            if (firstSequence_ == 0 && !block.empty()) {
...
ReplayReadResult readReplayFile(const std::filesystem::path& path,
    ...
    while (true) {
        ...
        if (header.type == kChunkMetadata) {
            ...
            if (!reader.u32(count)) {
                result.error = "Snapshot block is missing its count";
                return result;
            }
...
bool writeReplaySummary(const ReplayReadResult& replay,
    ...
    stream << "status=" << (replay.ok ? "OK" : "ERROR") << '\n';
    stream << "error=" << replay.error << '\n';
    stream << "engine_build=" << replay.metadata.engineBuild << '\n';
    stream << "git_revision=" << replay.metadata.gitRevision << '\n';
    stream << "xplane_version=" << replay.metadata.xplaneVersion << '\n';
    stream << "platform=" << replay.metadata.platform << '\n';
    stream << "aircraft_name=" << replay.metadata.aircraftName << '\n';
    stream << "aircraft_icao=" << replay.metadata.aircraftIcao << '\n';
    stream << "aircraft_relative_path=" << replay.metadata.aircraftRelativePath << '\n';
    stream << "snapshot_count=" << replay.summary.snapshotCount << '\n';
    stream << "dropped_snapshot_count=" << replay.summary.droppedSnapshotCount << '\n';
    stream << "first_sequence=" << replay.summary.firstSequence << '\n';
    stream << "last_sequence=" << replay.summary.lastSequence << '\n';
    stream << std::fixed << std::setprecision(6);
    stream << "first_simulator_time=" << replay.summary.firstSimulatorTimeSeconds << '\n';
    stream << "last_simulator_time=" << replay.summary.lastSimulatorTimeSeconds << '\n';
    stream << "clean_end_chunk=" << (replay.summary.cleanEndChunk ? 1 : 0) << '\n';
    if (!stream) {
